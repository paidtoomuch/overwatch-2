...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? c::accent : c::button::background_inactive, g.IO.DeltaTime * 6.f);
...
bool ImGui::Tab(bool selected, const char* label, const ImVec2& size_arg)
{
    ...
    it_anim->second.line = ImLerp(it_anim->second.line, selected ? c::accent : c::tab::background_inactive, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, selected || hovered ? c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);
    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2((size_arg.x - 13), (size_arg.y / 4)), bb.Max - ImVec2(8, (size_arg.y / 4)), GetColorU32(it_anim->second.line), c::button::rounding, ImDrawCornerFlags_Left);

    ImGui::GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 50, bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.text), label);
...
bool ImGui::SubTab(bool selected, const char* label, const ImVec2& size_arg)
{
    ...
    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2((size_arg.x / 4), (size_arg.y - 5)), bb.Max - ImVec2((size_arg.x / 4), 0), GetColorU32(it_anim->second.line), c::button::rounding, ImDrawCornerFlags_Top);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, GetColorU32(c::accent), 10.f);
...
{
    ImVec4 background = c::checkbox::background_inactive;
    ImVec4 checkmark = c::checkbox::checkmark_inactive;
    ImVec4 text = c::text::text_active;
};
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.checkmark = ImLerp(it_anim->second.checkmark, *v ? c::checkbox::checkmark_active : c::checkbox::checkmark_inactive, g.IO.DeltaTime * 6.f);
    ...
    if (label_size.x > 0.0f)
    {
        ...
        RenderText(ImVec2(check_bb.Max.x + 24, check_bb.Min.y - 2), label);
        ImGui::PopStyleColor();
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2(45, -7), window->DC.CursorPos + ImVec2(w + 45, 23));
    const ImRect total_bb(bb.Min, bb.Max + ImVec2(label_size.x, 0.f));
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, popup_open || hovered ? c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);

    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);

    window->DrawList->AddRectFilled(bb.Min, ImVec2(value_x2, bb.Max.y), GetColorU32(c::combo::background_inactive), c::combo::rounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersLeft);

    window->DrawList->AddRectFilled(ImVec2(value_x2, bb.Min.y), bb.Max, GetColorU32(c::combo::background_inactive), c::combo::rounding, (w <= arrow_size) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersRight);
    if (value_x2 + arrow_size <= bb.Max.x)
        RenderArrow(window->DrawList, ImVec2(value_x2, bb.Min.y + 9), GetColorU32(it_anim->second.text), ImGuiDir_Down, 0.7f);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(bb.Min + ImVec2(8, 3), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    PopStyleColor();

    if (label_size.x > 0.0f)
    {
        ImGui::PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
        RenderText(ImVec2(bb.Max.x + 15, bb.Min.y + 1), label);
        ImGui::PopStyleColor();
    }
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    PushStyleColor(ImGuiCol_PopupBg, GetColorU32(c::combo::background_inactive));
...
struct drag_state
{
    ...
    ImVec4 text_num = c::text::text_hide;
    ImVec4 text = c::text::text;
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    static std::map<ImGuiID, drag_state> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, drag_state() });
        it_anim = anim.find(id);
    }

    // This code is somewhat scuffed, but tl;dr, the custom imgui animations weren't in this function, which this function needs them, or else they wont render properly when using the custom imgui.
    it_anim->second.navigator = ImLerp(it_anim->second.navigator, IsItemActive() ? c::accent : hovered ? c::drag::background_faded : c::drag::background_inactive, g.IO.DeltaTime * 6.f);
    it_anim->second.text_num = ImLerp(it_anim->second.text_num, IsItemActive() ? c::text::text_active : hovered ? c::text::text_hov : c::text::text_faded, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(c::slider::background_inactive), c::slider::rounding);
    ...
    if (label_size.x > 0.0f)
        GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), GetColorU32(it_anim->second.text), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    it_anim->second.navigator = ImLerp(it_anim->second.navigator, IsItemActive() ? c::accent : hovered ? c::slider::background_inactive : c::slider::navigator_inactive, g.IO.DeltaTime * 6.f);

    it_anim->second.text_num = ImLerp(it_anim->second.text_num, IsItemActive() ? c::text::text_active : hovered ? c::text::text_hov : c::text::text_hide, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 0), ImVec2(it_anim->second.slow + frame_bb.Min.x + 14, grab_bb.Max.y + 1), GetColorU32(c::accent), c::slider::rounding);
    }
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);

    RenderArrow(GetWindowDrawList(), ImVec2(it_anim->second.slow + frame_bb.Min.x + 2, grab_bb.Max.y - 30), GetColorU32(it_anim->second.navigator), ImGuiDir_Down, 0.7f);

    GetWindowDrawList()->AddRectFilled(ImVec2(it_anim->second.slow + frame_bb.Min.x + 6 - (CalcTextSize(value_buf).x / 2), grab_bb.Min.y - 35), ImVec2(it_anim->second.slow + frame_bb.Min.x + 18 + (CalcTextSize(value_buf).x / 2), grab_bb.Max.y - 25), GetColorU32(it_anim->second.navigator), c::slider::rounding);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text_num));
    RenderTextClipped(frame_bb.Min + ImVec2(it_anim->second.slow - 20, -62), ImVec2(it_anim->second.slow + frame_bb.Min.x + 44, grab_bb.Max.y + 1), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    PopStyleColor();

    ImGui::PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderText(ImVec2(frame_bb.Max.x + 15, frame_bb.Min.y - 6), label);
    ImGui::PopStyleColor();
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    text << "frame size: " << frame_size.x << ", " << frame_size.y << std::endl;
    text << "label size: " << label_size.x << ", " << label_size.y << std::endl;
    text << "total size: " << total_size.x << ", " << total_size.y << std::endl;
    ImU32 color = 0xFFFFFFFF;
    window->DrawList->AddText(ImVec2(0, 0), color, text.str().c_str());
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, bg_color, c::textbox::rounding);
                }
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            GetWindowDrawList()->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                GetWindowDrawList()->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(), GetColorU32(ImGuiCol_Text));
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        window->DC.CursorPos = ImVec2(pos.x + button_offset_x + 45, pos.y);
        ...
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                const ImVec2 squarePosition = ImVec2(picker_active_window->DC.CursorPos.x + padding, picker_active_window->DC.CursorPos.y + padding);
                const ImVec2 squareSize = ImVec2(ImGui::GetContentRegionMax().x, ImGui::GetContentRegionMax().y);

                // Define the background color (ImU32 format)
                ImU32 backgroundColor = IM_COL32(15, 17, 20, 255); // Adjust the color as needed

                // Use AddRectFilled to render the background square
                picker_active_window->DrawList->AddRectFilled(squarePosition, ImVec2(squarePosition.x + squareSize.x + padding, squarePosition.y + squareSize.y + padding), ImGui::GetColorU32(c::child::background), c::bg::rounding);
                picker_active_window->DrawList->AddRect(squarePosition, ImVec2(squarePosition.x + squareSize.x + padding, squarePosition.y + squareSize.y + padding), ImGui::GetColorU32(c::bg::background), c::bg::rounding, 0, 3.0f);
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
    ...
    */
    ...
    if (Button("Copy as...", ImVec2(ImGui::CalcItemWidth() / 4, 30)))
        OpenPopup("Copy");
    if (BeginPopup("Copy"))
    {
        ...
        const ImVec2 squarePosition = ImVec2(window->DC.CursorPos.x + padding, window->DC.CursorPos.y + padding);
        const ImVec2 squareSize = ImVec2(ImGui::GetContentRegionMax().x, ImGui::GetContentRegionMax().y);
        ...
        // Use AddRectFilled to render the background square
        window->DrawList->AddRectFilled(squarePosition, ImVec2(squarePosition.x + squareSize.x - 1, squarePosition.y + squareSize.y), ImGui::GetColorU32(c::bg::background), c::bg::rounding);
        window->DrawList->AddRect(squarePosition, ImVec2(squarePosition.x + squareSize.x - 1, squarePosition.y + squareSize.y), ImGui::GetColorU32(c::bg::border), c::bg::rounding, 0, 1.0f);
